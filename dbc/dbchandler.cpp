...
//            qDebug() << "Found a BO line";
            ...
            if (match.hasMatch())
            {
                ...
                currentMessage->crcSignal = nullptr;
            }
            else numMsgFaults++;
        }
...
//                    qDebug() << "standard signal";
            ...
            if (match.hasMatch())
            {
                ...
                if(sig.name.endsWith("CRC")){
                    int cnt = currentMessage->sigHandler->getCount();
                    currentMessage->crcSignal = currentMessage->sigHandler->findSignalByIdx(cnt-1);;
                }
            }
            else numSigFaults++;
        }
